    ...
    forAll(mesh_.cells(), celli)
    {
        ...
        forAll(cellTets, tetI)
        {
            ...
            forAll(molecules, i)
            {
                ...
                label nParticlesToInsert = label(particlesRequired.getValue()); // codi:
    ...
    forAll(cellOccupancy_, celli)
    {
        ...
        if (nC > 1)
        {
            ...
            forAll(cellParcels, i)
            {
                ...
                label subCell =
                    (pos0(relPos.x()) + 2*pos0(relPos.y()) + 4*pos0(relPos.z())).getValue(); // codi:
            ...
            label nCandidates(selectedPairs.getValue()); // codi:
